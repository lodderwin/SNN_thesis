...
class NEAT(object):
    ...
    def __init__(self):
        ...
        self.div_training = [np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25)]
        self.wx_training = [np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25), np.random.uniform(0.15, 0.25)]
    def start_evolutionary_process(self, iterations=1):
        ...
        while i<iterations:
            ...
            # self.species[self.best_species].learn_best_genome(div_training, wx_training, self.species[self.best_species].best_genome)
            # self.best_genome = self.species[self.best_species].genomes[self.species[self.best_species].best_genome]
            # self.track_learning_cma.append(self.best_genome.fitness)
...
with open('testing_decreasedCMAES_2D_fast_test_nocma.pkl', 'wb') as outp:
    dill.dump(a, outp)
