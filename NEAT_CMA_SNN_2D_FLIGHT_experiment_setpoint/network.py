...
# a.species[0].find_all_routes(a.species[0].genomes[0])
    ...
    def is_compatible(self, comparison_genome):
        ...
        print('pay attention here', normalization_const, num_excess_genes,num_disjoint_genes,avg_weight_diff, compatibility_score)
        return compatible
    ...
    def mutate(self):
        ...
        if np.random.uniform() < config.ADD_GENE_MUTATION:
            ...
            while not gene_added:
                ...
                # neuron with least number of connections
                ...
                # sorted_neurons = [x for _, x in sorted(zip(p_lst, neurons_lst_order))]
                # index_chosen_neuron = self.get_skewed_random_sample(len(sorted_neurons), -0.5)
                # selected_output_node = sorted_neurons[index_chosen_neuron]
