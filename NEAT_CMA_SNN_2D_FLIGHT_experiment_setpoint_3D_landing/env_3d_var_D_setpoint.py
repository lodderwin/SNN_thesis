...
class LandingEnv3D:
    ...
    def __init__(
        ...
        self.sideways_reward = state_0[1]
    ...
    def x_prob(self):
        variable = self.state[3]/(2.*self.state[2]) - self.ref_wx
        if variable>0.:
            if variable>1.:
                variable = 1.
            prob = np.exp(-1./((variable)/0.5))/2. 
            return (prob, 0.)
        elif variable<0.:
            if variable<-1.:
                variable = -1.
            prob = np.exp(1./((1*variable)/0.5))/2. 
            return  (0., prob)

    def z_prob(self):
        variable = self.state[5]/(2.*self.state[2]) - self.ref_div
        if variable>0.:
            if variable>1.:
                variable = 1.
            prob = np.exp(-1./((variable)/0.5))/2. 
    ...
    def y_prob(self):
        variable = self.state[4]/(2.*self.state[2]) - self.ref_wy
        if variable>0.:
            if variable>1.:
                variable = 1.
            prob = np.exp(-1./((variable)/0.5))/2. 
    ...
    def step(self, act):
        ...
        if self.done:
            ...
            self.reward = self.reward + np.abs(self.state[4][0]*10)
    ...
    def _get_reward(self):
        ...
        if self.t<1.0:
            ...
            sideways_gain = (self.ref_wy*self.state[2][0]/2)*self.param["dt"]
            # print(forward_gain)
            ...
            sideways_reward = np.abs((self.state[1][0] - self.sideways_reward) - sideways_gain)   
            ...
            self.sideways_reward = copy.copy(self.state[1][0])
            # self.height_reward = self.state[2][0]  #something wrong with copy
            # self.forward_reward = self.state[0][0]
            return  forward_reward + height_reward + sideways_reward
