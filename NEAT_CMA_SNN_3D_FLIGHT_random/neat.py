...
#     a.species[0].create_random_network(genome_id, 30, 4) 
#     print(genome_id)

#     # create 10-15 random ones and mutate 
# # a.first_round_evolutionary_process_neat()
# a.start_evolutionary_process(iterations=1)
# with open('testing_decreasedCMAES_3D_new_control_sys_faster_random_30_4.pkl', 'wb') as outp:
#     dill.dump(a, outp)

# print("--- %s seconds ---" % (time.time() - start_time))


with open('testing_decreasedCMAES_3D_new_control_sys_faster_random_30_4.pkl', 'rb') as f:
    a = dill.load(f)
for i in range(25):
    start_time = time.time()
    a.start_evolutionary_process(iterations=1)
