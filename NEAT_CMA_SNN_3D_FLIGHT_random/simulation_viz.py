...
class objective:
    ...
    def visualize_spikes(self, spikes, t):
        ...
        fig.savefig('test25-10.png', dpi=100)  
...
####
    ...
    def objective_function_multiple_pos(self, model, ref_x_pos, ref_y_pos, ref_z_pos, runs):  # add prob here
        all_runs_vertical = []
        all_runs_horizontal = []
        ...
        self.ref_x_pos = ref_x_pos
        self.ref_y_pos = ref_y_pos
        self.ref_z_pos = ref_z_pos
        ax.scatter(self.ref_x_pos, self.ref_y_pos, self.ref_z_pos)
        for i in range(len(self.ref_x_pos)):
            ax.plot([0.0,self.ref_x_pos[i]], [0.0,self.ref_y_pos[i]], [2.5,self.ref_z_pos[i]], alpha=0.5)
        for run in range(runs):
        ...
        print('reward:', reward_cum, self.environment.t)
        # self.visualize_spikes(spikes, self.environment.t)
        plt.savefig('2510meeting_many.png')
        return reward_cum

with open('testing_decreasedCMAES_3D_new_control_sys_faster_random_30_4_really_goodone_withCMA_test.pkl', 'rb') as f:
    neat_class = dill.load(f)

range_coordinates_x = [(-1,1), (1, 4), (1, 4), (-1, -4), (-1, -4)]
range_coordinates_y = [(-1,1), (1, 4), (-1, -4), (-1, -4), (1, 4)]
range_coordinates_z = [(2., 3.), (1., 3.5), (1., 3.5), (1., 3.5), (1., 3.5)]
ref_x_pos = [np.random.uniform(range_coordinates_x[0][0], range_coordinates_x[0][1]), np.random.uniform(range_coordinates_x[1][0], range_coordinates_x[1][1]), np.random.uniform(range_coordinates_x[2][0], range_coordinates_x[2][1]), np.random.uniform(range_coordinates_x[3][0], range_coordinates_x[3][1]), np.random.uniform(range_coordinates_x[4][0], range_coordinates_x[4][1])]
ref_y_pos = [np.random.uniform(range_coordinates_y[0][0], range_coordinates_y[0][1]), np.random.uniform(range_coordinates_y[1][0], range_coordinates_y[1][1]), np.random.uniform(range_coordinates_y[2][0], range_coordinates_y[2][1]), np.random.uniform(range_coordinates_y[3][0], range_coordinates_y[3][1]), np.random.uniform(range_coordinates_y[4][0], range_coordinates_y[4][1])]
ref_z_pos = [np.random.uniform(range_coordinates_z[0][0], range_coordinates_z[0][1]), np.random.uniform(range_coordinates_z[1][0], range_coordinates_z[1][1]), np.random.uniform(range_coordinates_z[2][0], range_coordinates_z[2][1]), np.random.uniform(range_coordinates_z[3][0], range_coordinates_z[3][1]), np.random.uniform(range_coordinates_z[4][0], range_coordinates_z[4][1])]
...
objective.objective_function_multiple_pos(model, ref_x_pos, ref_y_pos, ref_z_pos, 10)
...
# environment = Quadhover()
# # objective = objective(model, environment=environment)
# objective.objective_function_multiple_pos(model, ref_x_pos, ref_y_pos, ref_z_pos, 1)
# # network_viz.view()
...
# new_weights, best_fitness = cma_es_class.optimize_run(cycles, ref_x_pos, ref_y_pos, ref_z_pos, neat_class.best_genome)
