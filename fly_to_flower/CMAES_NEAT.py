...
class CMA_ES:
    ...
    def optimize_run(self, runs, div, wx, wy, genome):
        ...
        counts_noprogress = 0
        for i in range(runs):
            print('gogogo', i, 'sigma', self.sigma)
            start_time = time.time() 
            for k in range(int(self.lamba)):
                self.arz[:,k] = np.random.normal(0., 0.05, size=(1,self.N)) #0.333  #does not necessarily have to fit column
                ...
                self.arfitness[k] = self.function(self.arx[:,k], div[0], wx[0], wy[0], genome) + self.function(self.arx[:,k], div[1], wx[1], wy[1], genome) + self.function(self.arx[:,k], div[2], wx[2], wy[2], genome) #+ self.function(self.arx[:,k], div[3], wx[3], wy[0], genome) + self.function(self.arx[:,k], div[4], wx[4], wy[0], genome) 
...
class CMA_ES_single:
    def __init__(
        ...
        self.lamba = int(4 + np.floor(4*np.log(self.N))) ####AAAAAANGEPAAAASTTT LET OP TODO: check dit even yo
        self.sigma = 0.5
        self.mu = np.floor(self.lamba/2)
        self.weights = np.log(self.mu+1) - np.log(np.arange(1, self.mu + 1)).reshape(-1,1)
        self.weights = self.weights/np.sum(self.weights)
    ...
    def optimize_run(self, runs, div, wx, wy, genome):
        ...
        for i in range(runs):
            ...
            for k in range(int(self.lamba)):
                ...
                self.arfitness[k] = self.function(self.arx[:,k], genome, div, wx, wy) 
