...
class NEAT(object):
    ...
    def __init__(self):
        ...
        self.track_learning = []
        self.track_learning_std = []
        self.best_genome_ever = []
        self.best_genome_ever_score = -10000000.

        self.best_species = None
        self.div_training = [np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]
        self.wx_training = [np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]
        self.wy_training = [np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]
    def start_evolutionary_process(self, iterations=1, cma_es_learning=False):
        i = 0
        ...
        while i<iterations:
            ...
            div_training = [np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]#, np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]
            wx_training = [np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]#, np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]
            wy_training = [np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11), np.random.uniform(0.09, 0.11)]
            # div_training = self.div_training
            # wx_training = self.wx_training
            for s_id, s in self.species.items():
                avg_fitness = s.run_generation(i, div_training, wx_training, wy_training, cma_es_learning)
                if avg_fitness != None:
            ...
            self.track_learning_std.append(np.std(np.asanyarray(list(avg_fitness_scores.values()))))
            if self.best_genome.fitness>self.best_genome_ever_score:
                self.best_genome_ever = self.best_genome
...
#     a.species[0].create_random_network(genome_id, 25, 6) 
#     print(genome_id)
# ## a.first_round_evolutionary_process_neat()
# a.start_evolutionary_process(iterations=1)
# with open('NEAT_3D_Landing_35_stdmultip_20_scaled_reward.pkl', 'wb') as outp:
#     dill.dump(a, outp)
...
for i in range(50):
    start_time = time.time()
    a.start_evolutionary_process(iterations=1, cma_es_learning=False)
    with open('NEAT_3D_Landing_35_stdmultip_20_scaled_reward.pkl', 'wb') as outp:
        dill.dump(a, outp)
    print("--- %s seconds ---" % (time.time() - start_time))
